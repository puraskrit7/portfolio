        ...
        {experience.map((company, idx) => (
          ...
          >
            <TiltCard className="rounded-2xl border border-border bg-panel/60 p-6 transition-colors hover:border-signal/30 sm:p-7">
              ...
              <div className="mt-2 font-mono text-xs text-faint">{company.location}</div>
                ...
                {company.roles.map((r, i) => (
                  <div key={i} className={i > 0 ? 'border-t border-border pt-6' : ''}>
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h3 className="font-display text-base text-ink">{r.role}</h3>
                      <span className="font-mono text-xs text-faint">{r.dateRange}</span>
                    </div>
                    ...
                    <ul className="mt-4 space-y-2">
                      {r.bullets.map((b, bi) => (
                        <li key={bi} className="flex gap-3 font-body text-sm leading-relaxed text-muted">
                    ...
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
                      <div className="flex flex-wrap gap-1.5">
                        {r.stack.map((s) => (
                          <span key={s} className="rounded-md bg-bg/60 px-2 py-1 font-mono text-[11px] text-muted">
                            {s}
                          </span>
                        ))}
                      </div>
                      {r.metric && (
                        <div className="shrink-0 text-right">
                          <div className="font-display text-lg text-signal">+{r.metric.value}</div>
                          <div className="font-mono text-[10px] uppercase tracking-wider text-faint">{r.metric.label}</div>
                        </div>
                      )}
                    </div>
                  </div>
