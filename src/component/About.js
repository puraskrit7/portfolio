...
const skills = [
  ...
  },
  {
    title: 'Testing & APIs',
    items: [
      'Jest', 'Unit Testing', 'TDD', 'RESTful APIs', 'GraphQL',
      'Axios', 'Postman', 'Salesforce APIs'
    ]
  },
  {
  ...
  return (
    <section className="about" id="about">
      <div className="about-container" data-aos="fade-up">
        <h2>About Me</h2>
        <p>
          I'm Puraskrit, a Front-End Developer with 4.5+ years of experience building scalable, performant UIs using ReactJS and TypeScript.
          I focus on clean code, accessibility, performance, and agile collaboration.
        </p>
      ...
      <div className="project-card" key={i}>
        ...
        <div className="project-tech">
          {proj.tech.map((techItem, tIdx) => (
            <span key={tIdx} className="chip tech-chip">{techItem}</span>
          ))}
        </div>
        <ul>
              ...
              <div className="skill-group" key={idx}>
                ...
                <div className="chip-group">
                  {group.items.map((item, sIdx) => (
                    <span className="chip" key={sIdx}>{item}</span>
                  ))}
                </div>
              </div>
            ))}
